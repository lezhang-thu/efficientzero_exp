...
class ReplayBuffer(object):
    ...
    def save_game(self, game, end_tag, gap_steps, priorities=None):
        ...
        if priorities is None:
            ...
            self.priorities = np.concatenate(
                (self.priorities, [max_prio for _ in range(valid_len)] +
    ...
    def prepare_batch_context(self, batch_size, beta):
        ...
        probs /= probs.sum()
        # sample data
        indices_lst = np.random.choice(total,
                                       batch_size,
                                       p=probs,
                                       replace=False)
    ...
    def _remove(self, num_excess_games):
        ...
        excess_games_steps = sum(
            [len(game) for game in self.buffer[:num_excess_games]])
    ...
    def info(self):
        total_transition = self.get_total_len()
        print("x: {}, y: {}".format(total_transition, self.transition_top))
