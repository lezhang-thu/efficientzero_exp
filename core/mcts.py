        ...
        with torch.no_grad():
            ...
            for index_simulation in range(self.config.num_simulations):
                ...
                last_actions = torch.from_numpy(
                    np.asarray(last_actions)).to(device).unsqueeze(1).long()
                ...
                value_prefix_pool = network_output.value_prefix.reshape(
                    -1).tolist()
