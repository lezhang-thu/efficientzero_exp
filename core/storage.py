...
class QueueStorage(object):
    def __init__(self, threshold=15, size=20):
        ...
        # debug - start
        self.queue = Queue(actor_options={"num_cpus": 3}, maxsize=size)
        #self.queue = Queue(actor_options={"num_cpus": 3, "resources": {'node:172.16.6.197': 0.01}}, maxsize=size)
        # debug - end
...
class SharedStorage(object):
    ...
    def set_data_worker_logs(self, eps_len, eps_len_max, eps_ori_reward,
                             eps_reward, eps_reward_max, temperature,
    ...
    def get_worker_logs(self):
        if len(self.reward_log) > 0:
            ...
            eps_lengths_max = sum(self.eps_lengths_max) / len(
                self.eps_lengths_max)
