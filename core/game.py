...
class GameHistory:
    ...
    def pad_over(self, next_block_observations, next_block_rewards,
                 next_block_root_values, next_block_child_visits):
        ...
        assert len(next_block_root_values
                   ) <= self.config.num_unroll_steps + self.config.td_steps
        assert len(next_block_rewards
    ...
    def obs(self, i, extra_len=0, padding=False):
        ...
        """
        frames = ray.get(self.obs_history)[i:i + self.stacked_observations +
                                           extra_len]
        if padding:
        ...
        if self.config.cvt_string:
            frames = [
                str_to_arr(obs, self.config.gray_scale) for obs in frames
            ]
        return frames
    ...
    def get_targets(self, i):
        ...
        return self.target_values[i], self.target_rewards[
            i], self.target_policies[i]
