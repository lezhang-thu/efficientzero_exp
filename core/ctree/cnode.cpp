...
void CNode::expand(int to_play, int hidden_state_index_x,
                   int hidden_state_index_y, float value_prefix,
...
void CRoots::prepare(float root_exploration_fraction,
                     ...
                     const std::vector<float> &value_prefixs,
...
void cback_propagate(std::vector<CNode *> &search_path,
                     tools::CMinMaxStats &min_max_stats, int to_play,
  ...
  for (int i = path_len - 1; i >= 0; --i) {
    ...
    if (i >= 1) {
      ...
      //                float qsa = (node->value_prefix - parent_value_prefix) +
      //                discount * node->value(); min_max_stats.update(qsa);
    }
...
void cbatch_back_propagate(int hidden_state_index_x, float discount,
                           const std::vector<float> &value_prefixs,
  ...
  for (int i = 0; i < results.num; ++i) {
    results.nodes[i]->expand(0, hidden_state_index_x, i, value_prefixs[i],
                             policies[i]);
    // reset
    results.nodes[i]->is_reset = is_reset_lst[i];

    cback_propagate(results.search_paths[i], min_max_stats_lst->stats_lst[i], 0,
                    values[i], discount);
  }
}
...
float cucb_score(CNode *child, tools::CMinMaxStats &min_max_stats,
                 float parent_mean_q, int is_reset,
                 float total_children_visit_counts, float parent_value_prefix,
