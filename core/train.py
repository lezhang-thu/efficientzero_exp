...
def update_weights(model,
    ...
    obs_batch_ori = torch.from_numpy(np.array(obs_batch_ori, copy=True)).to(
    ...
    # use GPU tensor
    action_batch = torch.from_numpy(np.array(action_batch, copy=True)).to(
        config.device).unsqueeze(-1).long()
    mask_batch = torch.from_numpy(np.array(mask_batch)).to(
        config.device).float()
    target_value_prefix = torch.from_numpy(np.array(target_value_prefix)).to(
        config.device).float()
    target_value = torch.from_numpy(np.array(target_value)).to(
        config.device).float()
    target_policy = torch.from_numpy(np.array(target_policy)).to(
        config.device).float()
    ...
    if vis_result:
        predicted_values, predicted_policies = scaled_value.detach().cpu(
        ), torch.softmax(policy_logits, dim=1).detach().cpu()

    # calculate the new priorities for each transition
    value_priority = L1Loss(reduction='none')(scaled_value.squeeze(-1),
                                              target_value[:, 0])
    ...
    if config.amp_type == 'torch_amp':
            ...
            for step_i in range(config.num_unroll_steps):
                ...
                if vis_result:
                    scaled_value_prefixs = config.inverse_reward_transform(
                        value_prefix.detach())
                    scaled_value_prefixs_cpu = scaled_value_prefixs.detach(
                    ).cpu()
                    ...
                    target_value_prefix_base = target_value_prefix_cpu[:,
                                                                       step_i].reshape(
                                                                           -1
                                                                       ).unsqueeze(
                                                                           -1)

                    other_loss[key] = metric_loss(scaled_value_prefixs_cpu,
        ...
        for step_i in range(config.num_unroll_steps):
            ...
            if vis_result:
                ...
                predicted_value_prefixs.append(scaled_value_prefixs_cpu)
                predicted_policies = torch.cat(
                    (predicted_policies, torch.softmax(policy_logits,
                                                       dim=1).detach().cpu()))
                state_lst = np.concatenate(
                    (state_lst, hidden_state.detach().cpu().numpy()))
    ...
    loss_data = (total_loss.item(), weighted_loss.item(), loss.mean().item(),
                 0, policy_loss.mean().item(), value_prefix_loss.mean().item(),
    ...
    if vis_result:
        ...
        # reward l1 loss
        value_prefix_indices_0 = (
            target_value_prefix_cpu[:, :config.num_unroll_steps].reshape(
                -1).unsqueeze(-1) == 0)
        value_prefix_indices_n1 = (
            target_value_prefix_cpu[:, :config.num_unroll_steps].reshape(
                -1).unsqueeze(-1) == -1)
        ...
        target_value_prefix_base = target_value_prefix_cpu[:, :config.
                                                           num_unroll_steps].reshape(
                                                               -1).unsqueeze(
                                                                   -1)

        predicted_value_prefixs = torch.stack(
            predicted_value_prefixs).transpose(1, 0).squeeze(-1)
        predicted_value_prefixs = predicted_value_prefixs.reshape(
            -1).unsqueeze(-1)
        other_loss['l1'] = metric_loss(predicted_value_prefixs,
                                       target_value_prefix_base)
        if value_prefix_indices_1.any():
        ...
        td_data = (new_priority, target_value_prefix.detach().cpu().numpy(),
...
def _train(model, target_model, replay_buffer, shared_storage, batch_storage,
    ...
    while step_count < config.training_steps + config.last_steps:
        ...
        if config.amp_type == 'torch_amp':
            log_data = update_weights(model, batch, optimizer, replay_buffer,
...
def train(config, summary_writer, model_path=None):
    ...
    #cpu_workers += [
    #    BatchWorker_CPU.options(resources={
    #        'node:172.16.6.197': 0.01,
    #        'node:172.16.6.223': 0.0
    #    }).remote(idx + config.cpu_actor, replay_buffer, storage, batch_storage, mcts_storage,
