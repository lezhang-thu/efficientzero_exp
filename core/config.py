...
class BaseConfig(object):
    def __init__(self,
                 ...
                 pred_hid: int = 64,
                 pred_out: int = 256,
                 value_support: DiscreteSupport = DiscreteSupport(-300,
                                                                  300,
                                                                  delta=1),
                 reward_support: DiscreteSupport = DiscreteSupport(-300,
                                                                   300,
                                                                   delta=1)):
    ...
    def scalar_transform(self, x):
        ...
        sign[x < 0] = -1.0
        output = sign * (torch.sqrt(torch.abs(x / delta) + 1) -
                         1) + epsilon * x / delta
        return output

    def inverse_reward_transform(self, reward_logits):
        return self.inverse_scalar_transform(reward_logits,
                                             self.reward_support)
    ...
    def inverse_scalar_transform(self, logits, scalar_support):
        ...
        sign[value < 0] = -1.0
        output = (((torch.sqrt(1 + 4 * epsilon *
                               (torch.abs(value) + 1 + epsilon)) - 1) /
                   (2 * epsilon))**2 - 1)
    ...
    def value_phi(self, x):
        return self._phi(x, self.value_support.min, self.value_support.max,
                         self.value_support.size)

    def reward_phi(self, x):
        return self._phi(x, self.reward_support.min, self.reward_support.max,
    ...
    def _phi(self, x, min, max, set_size: int):
        ...
        target.scatter_(2,
                        x_high_idx.long().unsqueeze(-1), p_high.unsqueeze(-1))
    ...
    def set_config(self, args):
        ...
        self.exp_path = os.path.join(args.result_dir, args.case, args.info,
                                     args.env, seed_tag, localtime)
