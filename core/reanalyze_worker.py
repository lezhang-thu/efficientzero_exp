...
class BatchWorker_CPU(object):
    ...
    def _prepare_reward_value_context(self, indices, games, state_index_lst,
                                      total_transitions):
        ...
        reward_value_context = [
            value_obs_lst, value_mask, state_index_lst, rewards_lst, traj_lens,
            td_steps_lst
        ]
    ...
    def _prepare_policy_re_context(self, indices, games, state_index_lst):
        ...
        # debug - end
        policy_re_context = [
            policy_obs_lst, policy_mask, state_index_lst, indices,
            child_visits, traj_lens
        ]
...
#@ray.remote(num_gpus=0.125, resources={'gpu_memory': 0.05, 'node:172.16.6.223': 0.05})
@ray.remote(num_gpus=0.125)
class BatchWorker_GPU(object):
    ...
    def _prepare_reward_value(self, reward_value_context):
        ...
        with torch.no_grad():
            ...
            for i in range(slices):
                ...
                m_obs = torch.from_numpy(value_obs_lst[beg_index:end_index]
            ...
            if self.config.use_root_value:
                ...
                roots = cytree.Roots(batch_size, self.config.action_space_size,
                                     self.config.num_simulations)
                noises = [
                    np.random.dirichlet([self.config.root_dirichlet_alpha] *
                                        self.config.action_space_size).astype(
                                            np.float32).tolist()
                    for _ in range(batch_size)
                ]
                roots.prepare(self.config.root_exploration_fraction, noises,
                              value_prefix_pool, policy_logits_pool)
                MCTS(self.config).search(roots, self.model, hidden_state_roots,
                                         reward_hidden_roots)
            ...
            value_lst = value_lst.reshape(-1) * (np.array(
                [self.config.discount
                 for _ in range(batch_size)])**td_steps_lst)
            ...
            for traj_len_non_re, reward_lst, state_index in zip(
                ...
                for current_index in range(
                    ...
                    for i, reward in enumerate(
                        ...
                        value_lst[
                            value_index] += reward * self.config.discount**i
    ...
    def _prepare_policy_re(self, policy_re_context):
        ...
        with torch.no_grad():
            ...
            for i in range(slices):
                ...
                m_obs = torch.from_numpy(policy_obs_lst[beg_index:end_index]
                                         ).to(device).float() / 255.0
                if self.config.amp_type == 'torch_amp':
                    with autocast():
                        m_output = self.model.initial_inference(m_obs)
            ...
            for state_index, game_idx in zip(state_index_lst, indices):
                ...
                for current_index in range(
                    ...
                    if policy_mask[policy_index] == 0:
                        ...
                        policy = [
                            visit_count / sum_visits
                            for visit_count in distributions
                        ]
    ...
    def _prepare_target_gpu(self):
        ...
        if input_countext is None:
            # debug - start
            print(
                "worker_id: {}, calling mcts_storage.pop returns None".format(
                    self.worker_id))
            # debug - end
            time.sleep(1)
        else:
